"""
alpha, gamma, LPAnnealingAdam Optim
objective: loss
- save sparsity, and save accuracy

two plots:
decay rates vs alpha
decay rates vs gamma
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(log_dir, "LPAnnealingAdamDecayRate6.log")),
...
results.to_csv(os.path.join(log_dir, "optimization_results6.csv"), index=False)
logger.info("Intermediate results saved to optimization_results6.csv")
...
try:
    ...
    plt.savefig(os.path.join(log_dir, "decay_rate_vs_alpha.png"))
    plt.close()
    logger.info("Plot saved: decay_rate_vs_alpha.png")
    ...
    plt.grid(True, which="both", ls="--")
    plt.savefig(os.path.join(log_dir, "decay_rate_vs_gamma.png"))
    plt.close()
    logger.info("Plot saved: decay_rate_vs_gamma.png")
